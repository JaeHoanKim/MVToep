...
```rmvRBF``` works similarly as ```rmvMat``` function. The only difference is the required arguments for the kernel setting. It is worth noting that rmvRBF is more likely to suffer from the ill-conditionedness of the covariance matrix due to its nature. One can use ```?RBFK``` to check how the covariance matrix is calculated.
